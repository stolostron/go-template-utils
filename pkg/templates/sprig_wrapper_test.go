...
func TestAvailableSprigFunctions(t *testing.T) {
	t.Parallel()
	...
	actual := AvailableSprigFunctions()

	if len(actual) != len(expected) {
		t.Fatalf("AvailableSprigFunctions length mismatch: expected %d, got %d", len(expected), len(actual))
	}

	for i, name := range expected {
		if actual[i] != name {
			t.Fatalf("AvailableSprigFunctions[%d] mismatch: expected %q, got %q", i, name, actual[i])
		}
	}
}
